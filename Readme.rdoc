...
- Create a +rails+ user. This is what Moonshine will use for it's operations. 

 server:~$ sudo adduser rails

- Give the +rails+ user sudo permisions

 server:~$ sudo visudo # add "rails ALL = NOPASSWD: ALL" below the "root ALL=(ALL) ALL" line

- Copy (via SCP) and install your local public key on the server

 local:~$ scp ~/.ssh/id_rsa.pub rails@server:/home/rails/my_key.pub
 local:~$ ssh rails@server

 server:~$ mkdir /home/rails/.ssh
 server:~$ tee -a my_key.pub /home/rails/.ssh/authorized_keys
 server:~$ sudo chown -R rails:rails /home/rails/.ssh
 server:~$ sudo chmod 700 /home/rails/.ssh
 server:~$ sudo chmod 600 /home/rails/.ssh/authorized_keys

- Make sure your server has access to your git repository (e.g. Github) via its public key.
...
- Edit the +config/moonshine.yml+ file, especially the +application+, +deploy_to+ and +repository+ keys.
- Next, capify your Rails app

 local:~$ capify .

- Edit the +config/deploy.rb+ file to reflect your server's IP address or hostname.
